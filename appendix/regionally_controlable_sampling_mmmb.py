...
def prepare_text(prompt, region_prompts, height, width, i):
    '''
    Args:
    ...
    # mmmb:
    print(f"Loading mmmb/{which_prompt}/merge_mask/bbox_info_{i}.txt")
    with open(f"mmmb/{which_prompt}/merge_mask/bbox_info_{i}.txt", 'r') as f:
        lines = f.readlines()
...
if __name__ == '__main__':
    args = parse_args()
    # mmmb:
    #which_prompt = "prompt_2"
    which_prompt = "prompt_0"
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

    num_images = args.num_image  # Number of images to generate

    # use this when no inference 
    # save_dir = os.path.join(args.save_dir, f'generated_images')
    ...
    for i in range(num_images):
        # mmmb: use different merge_mask
        args.sketch_condition = f"mmmb/{which_prompt}/merge_mask/merge_mask_{i}.png"
        print(f"Loading mmmb/{which_prompt}/merge_mask/merge_mask_{i}.png")
        ...
        input_prompt = [prepare_text(p, p_w, height, width, i) for p, p_w in zip(prompts, prompts_rewrite)]
        save_prompt = input_prompt[0][0]

        random_seed = i + args.seed
        ...
        image[0]=image[0].resize((int(final_W),int(final_H)))
