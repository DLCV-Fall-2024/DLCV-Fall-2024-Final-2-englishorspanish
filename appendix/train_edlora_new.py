...
from mixofshow_new.data.lora_dataset import LoraDataset
from mixofshow_new.data.prompt_dataset import PromptDataset
from mixofshow_new.pipelines.pipeline_edlora import EDLoRAPipeline, StableDiffusionPipeline
from mixofshow_new.pipelines.trainer_edlora import EDLoRATrainer
from mixofshow_new.utils.convert_edlora_to_diffusers import convert_edlora
from mixofshow_new.utils.util import MessageLogger, dict2str, reduce_loss_dict, set_path_logger
...
def train(root_path:str, args:argparse.Namespace):
    ...
    with accelerator.main_process_first():
        set_path_logger(accelerator, root_path, args.opt, opt, is_train=True, task_id = args.task_id)
    ...
    # Load model
    EDLoRA_trainer = EDLoRATrainer(task_id = args.task_id, 
                                   orthogonal = args.orthogonal if args.task_id > 1 else None,
                                   **opt['models'])
...
if __name__ == '__main__':
    ...
    # NOTE: 2 arguments are added for unifying path for all output paths
    parser.add_argument('--task_id', type=int, default=1,help='a unique id for a single concept training task')
    parser.add_argument('--orthogonal', type=float, default=0.1,help='weight for orthogonal loss')
