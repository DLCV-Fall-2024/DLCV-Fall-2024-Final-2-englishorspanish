...
from mixofshow_new.models.edlora import (LoRALinearLayer, revise_edlora_unet_attention_controller_forward,
                                     revise_edlora_unet_attention_forward)
from mixofshow_new.pipelines.pipeline_edlora import bind_concept_prompt
from mixofshow_new.utils.ptp_util import AttentionStore
...
class EDLoRATrainer(nn.Module):
    def __init__(
        ...
        # NOTE: add new variables
        task_id = None,
        orthogonal = None,
        output_dir = 'output'
    ):
        super().__init__()
        # NOTE: new feature, consider orthagonality during training
        #self.oldckpt = [torch.load(old_ckpt) for old_ckpt in old_ckpts]
        ...
        if orthogonal is not None:
            ...
            for id in range(task_id-1):
                old_ckpt=osp.join(output_dir, f'task_{id+1}', 'models', 'edlora_model-latest.pth')
                ckpt=torch.load(old_ckpt)['params']['unet']
                #print(ckpt)
                #raise NotImplementedError()
                up_param={key:value for key, value in ckpt.items() if 'lora_up' in key}
                old_ckpts.append(up_param)
            self.old_ckpts=old_ckpts
                ...
                if module.__class__.__name__ == where:
                    for child_name, child_module in module.named_modules():
                        if child_module.__class__.__name__ == 'Linear' or (child_module.__class__.__name__ == 'Conv2d' and child_module.kernel_size == (1, 1)):
                            ...
                            self.unet_lora_name.append(name + '.' + child_name+'.lora_up.weight')
                            params_list.extend(list(lora_module.parameters()))
    ...
    def forward(self, images, prompts, masks, img_masks):
        ...
        # NOTE Add orthogonal loss
        if self.orthogonal is not None and self.task_id > 1:
            loss_orth=[]
            loss_enhan=[]
            new_weight=[]
            # loss_lags=[]
            for i in range(len(self.unet_lora_name)):
                weight=self.unet_lora[i].lora_up.weight
                # loss_lag=self.lanrange(weight.t(), i, len(self.old_ckpts))
                # loss_lags.append(loss_lag)
                new_weight.append(weight)
            for task_id in range(len(self.old_ckpts)):
                ckpt = self.old_ckpts[task_id]
                for i in range(len(self.unet_lora_name)):
                    weight=new_weight[i]
                    if not ckpt[self.unet_lora_name[i]].is_cuda:
                        ckpt[self.unet_lora_name[i]]=ckpt[self.unet_lora_name[i]].to(weight.device)
                    old_weight=ckpt[self.unet_lora_name[i]]
                    orth=torch.abs(torch.matmul(weight, old_weight.t())).mean()
                    enhan=F.mse_loss(weight, torch.ones_like(weight).to(weight.device))
                    loss_orth.append(orth)
                    loss_enhan.append(enhan)
            # loss_lags=sum(loss_lags)/len(loss_lags)
            loss_orth=sum(loss_orth)/len(loss_orth)
            loss_enhan=sum(loss_enhan)/len(loss_enhan)
            loss=loss+self.orthogonal*loss_orth+self.orthogonal*loss_enhan
